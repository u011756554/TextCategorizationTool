...
public class GNetMine_DocTerm_TermTerm_Transductive extends TransductiveLearner{
    ...
    //Constructor
    public GNetMine_DocTerm_TermTerm_Transductive(){
    ...
    //Constructor
    public GNetMine_DocTerm_TermTerm_Transductive(int numIterations, double alphaDoc, double lambdaDocTerm){
    ...
    public void buildClassifier(Instances dataTrain, Instances dataTest){
        ...
        for(int inst=0;inst<(numTrain + numTest);inst++){
            ...
            for(int term=0;term<neighbors.size();term++){
                if(dDocTerm[inst] != 0){
                    ...
                    indVal.value = (double)neighbors.get(term).value / (double)dDocTerm[inst];
        ...
        Neighbor[] adjacencyListSTermTerm = new Neighbor[numTerms];
        for(int term=0;term<numTerms;term++){
