...
public class SupervisedInductiveClassification_TrainTest {
    ...
    public static  void learning(SupervisedInductiveConfiguration_TrainTest configuration){
        
        try{
            DataSource trainSource = new DataSource(configuration.getDataTrain()); //Carregando arquivo de Dados
            Instances dataOriginalTrain = trainSource.getDataSet();

            DataSource testSource = new DataSource(configuration.getDataTest()); //Carregando arquivo de Dados
            Instances dataOriginalTest = testSource.getDataSet();
            
            Attribute classAtt = null;

            classAtt = dataOriginalTrain.attribute(dataOriginalTrain.numAttributes()-1); //Setting the last feature as class
            dataOriginalTrain.setClass(classAtt);
            
            classAtt = dataOriginalTest.attribute(dataOriginalTest.numAttributes()-1); //Setting the last feature as class
            dataOriginalTest.setClass(classAtt);
            ...
            outputFile.append("/");
            outputFile.append(dataOriginalTrain.relationName());
            ...
            if(configuration.isNB()){
                ...
                learning(configuration, threads, classifiers, dataOriginalTrain, dataOriginalTest, outputFilePar.toString(), numClasses);
            }
            ...
            if(configuration.isSMO()){
                ...
                if(parametersSMO.isLinearKernel()){
                    ...
                    for(int c=0;c<parametersSMO.getvalueesC().size();c++){
                        ...
                        learning(configuration, threads, classifiers, dataOriginalTrain, dataOriginalTest, outputFilePar2.toString(), numClasses);
                    }
                }
                ...
                for(int viz=0;viz<parametersKNN.getNeighbors().size();viz++){
                    ...
                    if(parametersKNN.getCosine()){
                        ...
                        if(parametersKNN.isWeighted()){
                            ...
                            learning(configuration, threads, classifiers, dataOriginalTrain, dataOriginalTest, outputFilePar4.toString(), numClasses);    
                        }
                        if(parametersKNN.isUnweighted()){
                            ...
                            learning(configuration, threads, classifiers, dataOriginalTrain, dataOriginalTest, outputFilePar3.toString(), numClasses);
                        }
                    }
            ...
            if(configuration.isBLR()){
                ...
                if(parametersBLR.getPriors().contains(1)){
                    ...
                    learning(configuration, threads, classifiers, dataOriginalTrain, dataOriginalTest, outputFilePar.toString() + 1 + "_Gaussian_", numClasses);
                } 
                if(parametersBLR.getPriors().contains(2)){
                    ...
                    learning(configuration, threads, classifiers, dataOriginalTrain, dataOriginalTest, outputFilePar.toString() + 2 + "_Laplacian_", numClasses);
                }
            }
    ...
    public static void learning(final SupervisedInductiveConfiguration_TrainTest configuration, final ExecutorService threads, final Classifier[][] classifiers, final Instances dataOriginalTrain, final Instances dataOriginalTest, final String outputFile, final int numClasses){
        ...
        try{
            ...
            Enumeration enumeration = dataOriginalTrain.classAttribute().enumerateValues();
            ...
            if(outputTemp.exists()){
                ...
                results = new Results2(output, 1, 1, "InductiveSupervised", classes);
            }
            
            //System.out.println("Output: " + output.getAbsolutePath());
            final Instances dataTrain = new Instances(dataOriginalTrain);
            dataTrain.randomize(new Random(0));
            final Instances dataTest = new Instances(dataOriginalTest);
            ...
            Thread thread = new Thread(new Runnable() {
                ...
                public void run() {
                    ...
                    try{
                        classifiers[0][0].buildClassifier(dataTrain);
                    }catch(OutOfMemoryError e){
                        configuration.getEmail().getContent().append("OutOfMemory!!!!");
                        configuration.getEmail().getContent().append(configuration.toString());
                        configuration.getEmail().send();
                    ...
                    Evaluations.SupervisedInductiveEvaluation(classifiers[0][0], dataTest, confusionMatrix);
